...
const getCityByRef = document.querySelector('#getCity')

getCityByRef.addEventListener('click', async (ev) => {
...
weatherForm.addEventListener('submit', async (ev) => {
    ...
    if (city !== '') {
        const {coord: {lon, lat}} = await getLongLatFromCity(city);
...
async function getLongLatFromCity(city) {
